...
export DOC_PREPROCESS_CHUNK_FILE="${DOC_PREPROCESS_CHUNK_FILE:-${DOC_PREPROCESS_CODE_DIR}/chunk_for_milvus.py}"
export DOC_PREPROCESS_OUTPUT_JSONL="${DOC_PREPROCESS_OUTPUT_JSONL:-${DOC_PREPROCESS_OUTPUT_DIR}/preprocessed.jsonl}"
export DOC_PREPROCESS_CHUNK_OUTPUT_JSONL="${DOC_PREPROCESS_CHUNK_OUTPUT_JSONL:-${DOC_PREPROCESS_OUTPUT_DIR}/chunks.jsonl}"
export DOC_PREPROCESS_METADATA_DEFINITION_FILE="${DOC_PREPROCESS_METADATA_DEFINITION_FILE:-${DOC_PREPROCESS_CODE_DIR}/metadata_definition.json}"
export DOC_PREPROCESS_CHUNK_SIZE="${DOC_PREPROCESS_CHUNK_SIZE:-1000}"
export DOC_PREPROCESS_CHUNK_OVERLAP="${DOC_PREPROCESS_CHUNK_OVERLAP:-150}"
export DOC_PREPROCESS_CHUNK_INCLUDE_OCR="${DOC_PREPROCESS_CHUNK_INCLUDE_OCR:-true}"
...
if [[ ! -f "${DOC_PREPROCESS_CHUNK_FILE}" ]]; then
  echo "ERROR: Chunk generation script file not found: ${DOC_PREPROCESS_CHUNK_FILE}" >&2
  exit 1
fi

if [[ ! -f "${DOC_PREPROCESS_METADATA_DEFINITION_FILE}" ]]; then
  echo "ERROR: Metadata definition file not found: ${DOC_PREPROCESS_METADATA_DEFINITION_FILE}" >&2
  exit 1
fi
...
CHUNK_CMD=(
  "${VENV_PY}" "${DOC_PREPROCESS_CHUNK_FILE}"
  "--input-jsonl" "${DOC_PREPROCESS_OUTPUT_JSONL}"
  "--chunk-output-jsonl" "${DOC_PREPROCESS_CHUNK_OUTPUT_JSONL}"
  "--metadata-definition-file" "${DOC_PREPROCESS_METADATA_DEFINITION_FILE}"
  "--chunk-size" "${DOC_PREPROCESS_CHUNK_SIZE}"
  "--chunk-overlap" "${DOC_PREPROCESS_CHUNK_OVERLAP}"
  "--include-ocr" "${DOC_PREPROCESS_CHUNK_INCLUDE_OCR}"
  "--required-python" "${DOC_PREPROCESS_REQUIRED_PYTHON}"
)

"${CHUNK_CMD[@]}"

if [[ ! -s "${DOC_PREPROCESS_CHUNK_OUTPUT_JSONL}" ]]; then
  echo "ERROR: Chunk generation produced no output: ${DOC_PREPROCESS_CHUNK_OUTPUT_JSONL}" >&2
  exit 1
fi
...
echo "SUCCESS: Milvus-ready chunk file written: ${DOC_PREPROCESS_CHUNK_OUTPUT_JSONL}"
